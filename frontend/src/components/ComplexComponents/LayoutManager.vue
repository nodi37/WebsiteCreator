...
export default {
  ...
  methods: {
    ...
    addComponentToLayout(component) {
      ...
      for (const propsArr in component.props) {
        component.props[propsArr].forEach((el) => {
          propsToPush[el.name] = {
            type: propsArr,
            value: propsArr.toLowerCase() === "array" ? [] : "",
          };
        });
      }
    ...
    setSnackBarMainState: function (newValues) {
      ...
      const counter =
        newValues.count > 1 ? `${newValues.current}/${newValues.count}...` : "";
...
<template>
  <div class="w-full flex flex-col gap-6">
    ...
    <template v-if="isLoaded">
      ...
      <dialog-box
        v-model="addComponentsDialogVisible"
