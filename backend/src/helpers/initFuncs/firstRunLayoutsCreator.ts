...
//Temporary things, creates my cv and menu, to remove!
...
async function firstRunLayoutsCreator() {
    try {
        ...
        if (!topLayout) {
            ...
            //Temporary things, creates my cv and menu, to remove!
            const navCompData = await saveNewComponent(menu);
            //Temp


            await saveNewLayout({
                name: "topLayout",
                components: [navCompData._id], //Switch to empty arr
                isGlobal: true,
                isRequired: true
            });
            ...
            //Temporary things, creates my cv and menu, to remove!
            const homeCompsIds = await createCvComponents();
            //Temp
...
//Temporary things, creates my cv and menu, to remove!
async function createCvComponents() {
    let homeCompsIds = [];
